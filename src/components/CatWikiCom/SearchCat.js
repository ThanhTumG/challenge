...
import { increaseVoteBreed } from "../../api/CatAPI";


export default function SearchCat() {
    const { listCat, setMode, setSelectedCat } = useContext(AppContext)
    ...
    const handleOnclickCat = async (result) => {
        const theChosenCat = listCat.find((cat) => cat.name === result)
        setSelectedCat(theChosenCat);
        increaseVoteBreed(theChosenCat.reference_image_id);
        setInputText('')
        setResults([])
        setMode("detail")
    }
            ...
            <div className="absolute rounded-[14px] md:rounded-[24px] max-h-[220px] top-[80px] bg-white z-50 flex items-center flex-col overflow-y-auto  justify-start w-[100%]">
                ...
                {results.map((result, index) => (
                    ...
                    font-[500] text-[16px] hover:bg-[#979797]/[.2]">
                        <text className="capitalize font-montserrat font-[500] text-start text-[12px] md:text-[18px]">{result}</text>
                    </button>
                ))}
            </div>
        </div>
