...
import { Public, ChevronRight, Search } from "@mui/icons-material";
import listLocation from "../assets/location.json"

export default function TopSearchCat({ mostSearch, listCat }) {
    ...
    return (
        <div className="flex flex-col pt-10 w-full font-montserrat text-[#291507] space-y-4 md:space-y-10">
            ...
            <ul className="flex flex-col space-y-6 md:space-y-10 w-[90%] items-start">
                {mostSearch && mostSearch.map((cat, index) => {
                    const catIf = listCat.find((ele) => ele.reference_image_id === cat.image.id)
                    return (<li className="flex items-start space-x-7 md:space-x-12">
                        ...
                        <div className="flex flex-col items-start space-y-2 md:space-y-5">
                            {/* {const cat = listCat.find((ele) => ele.reference_image_id === cat.image.id)} */}
